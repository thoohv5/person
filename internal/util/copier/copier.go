...
func Copy(toValue interface{}, fromValue interface{}) (err error) {
	return copier(toValue, fromValue, Option{IgnoreEmpty: true})
}
...
func copier(toValue interface{}, fromValue interface{}, opt Option) (err error) {
	...
	for i := 0; i < amount; i++ {
		...
		if source.IsValid() {
			...
			// default dest field value
			for _, field := range deepFields(toType) {
				name := field.Name
				_, destFieldName := getFieldName(name, flgs)
				if toField := dest.FieldByName(destFieldName); toField.IsValid() && toField.CanSet() {
					dv, ok := field.Tag.Lookup("default")
					if !ok {
						continue
					}
					rv := reflect.ValueOf(field.Tag.Get("default"))
					if dv == "[]" {
						rv = reflect.ValueOf(make([]interface{}, 0))
					}
					set(toField, rv, opt.DeepCopy, converters)
				}
			}
			...
			// dest method deal data
			for _, field := range deepFields(toType) {
				name := field.Name
				_, destFieldName := getFieldName(name, flgs)

				var destMethod reflect.Value
				_destFieldName := fmt.Sprintf("%sAttribute", destFieldName)
				if dest.CanAddr() {
					destMethod = dest.Addr().MethodByName(_destFieldName)
				} else {
					destMethod = dest.MethodByName(_destFieldName)
				}

				if destMethod.IsValid() && !shouldIgnore(destMethod, opt.IgnoreEmpty) {
					toField := dest.FieldByName(destFieldName)
					params := []reflect.Value{toField, reflect.ValueOf(field.Tag)}
					values := destMethod.Call(params[0:destMethod.Type().NumIn()])
					if len(values) >= 1 && toField.IsValid() && toField.CanSet() {
						set(toField, values[0], opt.DeepCopy, converters)
					}
				}
			}
