...
import (
	...
	"github.com/thoohv5/person/pkg/config"
)
...
func NewSource(path string, suffix string) config.Source {
	return &file{path: path, suffix: suffix}
}
...
func (f *file) loadDir(path string) (kvs []*config.KeyValue, err error) {
	...
	for _, file := range files {
		// ignore hidden files
		if file.IsDir() || strings.HasPrefix(file.Name(), ".") || (len(f.suffix) > 0 && !strings.HasSuffix(file.Name(), f.suffix)) {
			continue
		}
